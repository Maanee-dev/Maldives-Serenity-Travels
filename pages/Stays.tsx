    ...
    <div className="bg-[#FCFAF7] min-h-screen">
      <section className="pt-56 pb-24 md:pb-40 px-6 text-center reveal active">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="h-1.5 w-24 bg-amber-400 mx-auto mb-16"></div>
      ...
      {/* Enhanced Search Input with stronger borders and visible text */}
        ...
        <div className="relative group max-w-4xl mx-auto">
          ...
          <div className="relative">
            <input 
              ...
              className="w-full bg-white border-2 border-slate-400 rounded-3xl px-8 py-8 text-xl md:text-3xl font-serif italic text-slate-950 outline-none transition-all focus:border-slate-950 focus:ring-4 focus:ring-slate-100 placeholder:text-slate-400"
            />
            <div className="absolute right-8 top-1/2 -translate-y-1/2 text-slate-950">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 pb-48">
        <div className="flex flex-col gap-24">
          {/* Enhanced Filter Bar with better visibility */}
          <div className="flex flex-col md:flex-row justify-between items-center gap-12 border-b-2 border-slate-300 pb-16 reveal active">
            <div className="flex gap-4 p-2 bg-slate-200 rounded-full border-2 border-slate-400">
              <button 
                onClick={() => setStayType(AccommodationType.RESORT)}
                className={`px-8 md:px-12 py-3.5 rounded-full text-[11px] font-black transition-all duration-500 uppercase tracking-[0.3em] ${stayType === AccommodationType.RESORT ? 'bg-slate-950 text-white shadow-xl' : 'text-slate-700 hover:text-slate-950'}`}
              >
                Resorts
              </button>
              <button 
                onClick={() => setStayType(AccommodationType.GUEST_HOUSE)}
                className={`px-8 md:px-12 py-3.5 rounded-full text-[11px] font-black transition-all duration-500 uppercase tracking-[0.3em] ${stayType === AccommodationType.GUEST_HOUSE ? 'bg-slate-950 text-white shadow-xl' : 'text-slate-700 hover:text-slate-950'}`}
              >
                Local Islands
              </button>
            </div>
            ...
            <div className="flex flex-wrap justify-center md:justify-end gap-10 md:gap-16 items-center">
              <div className="flex flex-col gap-3">
                <span className="text-[10px] font-black text-slate-950 uppercase tracking-widest">Region</span>
                <select 
                  value={selectedAtoll}
                  onChange={(e) => setSelectedAtoll(e.target.value)}
                  className="bg-white border-2 border-slate-400 rounded-xl px-4 py-2 text-[11px] font-black uppercase tracking-widest text-slate-950 outline-none cursor-pointer hover:border-slate-950 transition-all focus:ring-2 focus:ring-sky-100"
                >
              ...
              <div className="flex flex-col gap-3">
                <span className="text-[10px] font-black text-slate-950 uppercase tracking-widest">Arrival</span>
                <select 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-16 min-h-[400px]">
            {loading ? (
              <div className="col-span-full py-40 text-center">
                <div className="w-12 h-12 border-4 border-slate-200 border-t-sky-600 rounded-full animate-spin mx-auto mb-8"></div>
                <p className="text-[11px] font-black text-slate-900 uppercase tracking-[0.6em]">Consulting the archives...</p>
              </div>
              ...
              <div className="col-span-full py-40 text-center reveal active">
                <h3 className="text-4xl font-serif font-bold italic text-slate-950 mb-6">No Sanctuaries Found.</h3>
                <p className="text-slate-800 uppercase tracking-widest text-[11px] font-black">Adjust your boutique filters to continue the journey.</p>
              </div>
            )}
          </div>

          {!loading && totalPages > 1 && (
            <div className="mt-12 flex flex-col items-center gap-10 reveal active border-t-2 border-slate-300 pt-16">
              <div className="flex items-center justify-between w-full max-w-sm md:max-w-none md:gap-16">
                <button 
                  onClick={() => handlePageChange(currentPage - 1)}
                  disabled={currentPage === 1}
                  className={`flex items-center gap-4 text-[11px] font-black uppercase tracking-[0.4em] transition-all duration-500 ${currentPage === 1 ? 'opacity-20 cursor-not-allowed' : 'text-slate-950 hover:text-sky-600'}`}
                >
                ...
                <div className="hidden md:flex items-center gap-4 md:gap-6">
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                    <button
                      ...
                      className={`text-[11px] font-black uppercase tracking-widest w-12 h-12 rounded-full border-2 transition-all duration-500 ${currentPage === page ? 'bg-slate-950 text-white border-slate-950 shadow-xl' : 'text-slate-900 border-slate-400 hover:border-slate-950 hover:text-slate-950'}`}
                    >
                      {page}
                    </button>
                  ))}
                ...
                <button 
                  ...
                  className={`flex items-center gap-4 text-[11px] font-black uppercase tracking-[0.4em] transition-all duration-500 ${currentPage === totalPages ? 'opacity-20 cursor-not-allowed' : 'text-slate-950 hover:text-sky-600'}`}
                >
