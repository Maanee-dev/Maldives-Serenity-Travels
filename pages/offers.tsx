...
const Offers: React.FC = () => {
  ...
  const StarRating = ({ count }: { count: number }) => (
    <div className="flex gap-0.5 mb-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg 
          key={i} 
          className={`w-4 h-4 ${i < count ? 'text-amber-400 fill-current' : 'text-slate-200 dark:text-slate-800 fill-current transition-colors'}`} 
          viewBox="0 0 24 24"
        >
  ...
  return (
    <div className="bg-parchment dark:bg-slate-950 min-h-screen selection:bg-sky-100 selection:text-sky-900 pb-32 transition-colors duration-700">
       ...
       {/* Intelligence Filter Bar - Deep Dark Pass */}
       <section className="max-w-7xl mx-auto px-6 -mt-16 md:-mt-24 mb-20 sticky top-24 z-50 transition-all duration-700">
          <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-100 dark:border-white/5 rounded-[3rem] p-4 md:p-6 shadow-2xl flex flex-col lg:flex-row gap-6 items-center">
            ...
            <div className="flex-1 w-full relative">
              <input 
                ...
                className="w-full bg-slate-50 dark:bg-slate-800/50 border-none rounded-full px-8 py-4 text-[10px] font-black uppercase tracking-widest focus:ring-2 focus:ring-sky-500 transition-all placeholder:text-slate-300 dark:text-white dark:placeholder:text-slate-600"
              />
            </div>

            {/* Duration Selector */}
            <div className="flex items-center gap-4 px-4 border-l border-slate-100 dark:border-white/5 hidden lg:flex">
              <span className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Duration</span>
            ...
            <div className="flex items-center gap-4 px-4 border-l border-slate-100 dark:border-white/5 hidden lg:flex">
               {categories.map(cat => (
                 <button 
                  ...
                  className={`text-[9px] font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'text-sky-500 dark:text-sky-400' : 'text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
                 >
                   {cat}
                 </button>
               ))}
                  ...
                  <div key={offer.id} className="group flex flex-col h-full reveal active">
                    ...
                    <div className="relative aspect-[3.5/4] rounded-[2.5rem] overflow-hidden mb-8 shadow-sm group-hover:shadow-2xl transition-all duration-1000 bg-slate-100 dark:bg-slate-900">
                       ...
                       <div className="absolute top-6 left-6">
                          <div className="bg-amber-400 dark:bg-amber-500 text-slate-950 dark:text-white px-4 py-1.5 rounded-full flex items-center gap-2 shadow-lg">
              ...
              {totalPages > 1 && (
                <div className="mt-32 flex justify-center items-center gap-4">
                  <button 
                    ...
                    className="w-12 h-12 rounded-full border border-slate-200 dark:border-white/10 flex items-center justify-center hover:bg-slate-950 dark:hover:bg-white hover:text-white dark:hover:text-slate-950 transition-all disabled:opacity-20 text-slate-950 dark:text-white"
                  >
                    &larr;
                  </button>
                  ...
                  <div className="flex gap-2">
                    {Array.from({ length: totalPages }).map((_, i) => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-full text-[10px] font-black transition-all ${currentPage === i + 1 ? 'bg-slate-950 dark:bg-white text-white dark:text-slate-950 shadow-lg' : 'bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 text-slate-400 hover:border-slate-300 dark:hover:border-white/20'}`}
                      >
                        {i + 1}
                      </button>
                    ))}
