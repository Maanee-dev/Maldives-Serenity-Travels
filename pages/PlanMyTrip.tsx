    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#FCFAF7]">
        <div className="text-center max-w-2xl reveal active">
           ...
           <p className="text-slate-400 uppercase tracking-[0.4em] text-[10px] leading-[2.5] mb-16 font-medium max-w-md mx-auto">
    ...
    <div className="bg-[#FCFAF7] min-h-screen py-64 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-40 reveal">
          ...
          <div className="h-px w-24 bg-amber-400 mx-auto mb-16"></div>
          <p className="text-slate-400 text-[11px] font-bold uppercase tracking-[0.5em] leading-[2.5] max-w-xl mx-auto opacity-80">
          ...
          <div className="flex justify-between items-center mb-32 px-4">
             {[1, 2, 3].map(i => (
               <div key={i} className="flex flex-col items-center gap-6 group">
                  <div className={`w-2 h-2 rounded-full transition-all duration-1000 ${step >= i ? 'bg-slate-950 scale-[2]' : 'bg-slate-100'}`}></div>
                  <span className={`text-[8px] font-bold uppercase tracking-widest transition-all duration-700 ${step === i ? 'text-slate-900 opacity-100' : 'text-slate-300 opacity-0'}`}>Phase {i}</span>
               </div>
             ))}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-24">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {['Honeymoon', 'Family Holiday', 'Diving Expedition', 'Pure Relaxation'].map(opt => (
                    <button 
                      ...
                      type="button" 
                      onClick={nextStep}
                      className="text-center p-12 rounded-[3.5rem] bg-white border border-slate-50 hover:border-slate-950 hover:shadow-2xl transition-all duration-700 group relative overflow-hidden"
                    >
            ...
            {step === 2 && (
              <div className="space-y-16 reveal">
                ...
                <div className="space-y-12">
                  <div className="border-b border-slate-100 py-6 transition-all focus-within:border-slate-950">
                    <label className="block text-[9px] font-bold text-slate-300 uppercase tracking-[0.6em] mb-6">Investment Range</label>
                  ...
                  <div className="border-b border-slate-100 py-6 transition-all focus-within:border-slate-950">
                    <label className="block text-[9px] font-bold text-slate-300 uppercase tracking-[0.6em] mb-6">Travel Window</label>
                    <input type="text" placeholder="E.G. LATE SEPTEMBER" className="w-full bg-transparent text-slate-950 font-bold uppercase tracking-[0.4em] text-xs md:text-sm focus:outline-none placeholder:text-slate-100" />
                  </div>
                </div>
                <div className="flex gap-16 pt-16">
                   <button type="button" onClick={prevStep} className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.5em] hover:text-slate-950 transition-colors">Back</button>
            ...
            {step === 3 && (
              <div className="space-y-16 reveal">
                ...
                <div className="space-y-12">
                  <div className="border-b border-slate-100 py-6 transition-all focus-within:border-slate-950">
                    <label className="block text-[9px] font-bold text-slate-300 uppercase tracking-[0.6em] mb-6">Full Name</label>
                    <input type="text" placeholder="LEAVE BLANK FOR ANONYMOUS INITIAL INQUIRY" required className="w-full bg-transparent py-2 text-xs md:text-sm font-bold uppercase tracking-[0.4em] focus:outline-none text-slate-950 placeholder:text-slate-100" />
                  </div>
                  <div className="border-b border-slate-100 py-6 transition-all focus-within:border-slate-950">
                    <label className="block text-[9px] font-bold text-slate-300 uppercase tracking-[0.6em] mb-6">Email Signature</label>
                    <input type="email" placeholder="WHERE TO SEND THE PORTFOLIO" required className="w-full bg-transparent py-2 text-xs md:text-sm font-bold uppercase tracking-[0.4em] focus:outline-none text-slate-950 placeholder:text-slate-100" />
                  </div>
                  <div className="border-b border-slate-100 py-6 transition-all focus-within:border-slate-950">
                    <label className="block text-[9px] font-bold text-slate-300 uppercase tracking-[0.6em] mb-6">Manifesto & Notes</label>
                    <textarea placeholder="ANY SPECIFIC ISLANDS OR PREFERENCES" rows={3} className="w-full bg-transparent py-2 text-xs md:text-sm font-bold uppercase tracking-[0.2em] focus:outline-none text-slate-950 placeholder:text-slate-100 leading-loose"></textarea>
                  </div>
                </div>
                <div className="flex gap-16 pt-16">
                   <button type="button" onClick={prevStep} className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.5em] hover:text-slate-950 transition-colors">Back</button>
        ...
        <div className="mt-48 text-center opacity-40">
           <p className="text-slate-300 text-[9px] font-bold uppercase tracking-[0.8em]">
