...
const Home: React.FC = () => {
  ...
  return (
    <div className="bg-[#FCFAF7] selection:bg-sky-100 selection:text-sky-900 overflow-x-hidden">
      ...
      {/* THE JOURNAL - OVERHAULED FOR MOBILE EDITORIAL FEEL */}
      <section className="py-24 md:py-48 bg-[#FCFAF7] overflow-hidden">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-20">
          <div className="text-center md:text-left mb-16 md:mb-24 reveal">
            ...
            <h2 className="text-4xl md:text-7xl lg:text-8xl font-serif font-bold text-slate-950 italic tracking-tighter">Editorial Dispatches.</h2>
          </div>
          
          {/* Mobile Cinematic Scroller / Desktop Asymmetric Grid */}
          <div className="relative">
            <div className="flex md:grid md:grid-cols-3 gap-10 md:gap-16 lg:gap-24 overflow-x-auto md:overflow-x-visible no-scrollbar snap-x snap-mandatory -mx-6 px-6 md:mx-0 md:px-0 pb-16 md:pb-0">
              {recentStories.map((post, idx) => (
                <Link 
                  key={post.id} 
                  to={`/stories/${post.slug}`} 
                  className={`group flex-shrink-0 w-[82vw] md:w-auto snap-center reveal relative transition-all duration-700 ${idx === 1 ? 'md:translate-y-12' : ''}`}
                  style={{ transitionDelay: `${idx * 150}ms` }}
                >
                  <div className="relative aspect-[3/4.5] md:aspect-[3/4] rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden mb-8 md:mb-12 shadow-sm transition-all duration-1000 group-hover:shadow-2xl group-hover:-translate-y-2 bg-slate-100">
                    <img src={post.image} alt={post.title} className="w-full h-full object-cover transition-transform duration-[6s] group-hover:scale-105" />
                    
                    {/* Editorial Sequence Number Overlay (Mobile Focused) */}
                    <div className="absolute top-8 right-10 opacity-30 group-hover:opacity-50 transition-opacity pointer-events-none">
                      <span className="text-[7rem] md:text-[8rem] font-serif italic text-white leading-none tracking-tighter select-none">0{idx + 1}</span>
                    </div>

                    {/* Category Label - Floating Magazine Style */}
                    <div className="absolute top-10 left-10">
                      <span className="bg-white/95 backdrop-blur-md px-6 py-2.5 rounded-full text-[9px] font-black text-slate-900 uppercase tracking-[0.5em] shadow-xl border border-white/20">
                        {post.category}
                      </span>
                    </div>
                  ...
                  <div className="px-6 relative">
                    <div className="flex items-center gap-4 mb-6">
                       <span className="text-slate-400 font-bold text-[9px] uppercase tracking-[0.6em] whitespace-nowrap">
                        {new Date(post.date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                      </span>
                      <div className="h-[1px] flex-grow bg-slate-100 group-hover:bg-sky-500 transition-colors duration-1000"></div>
                    </div>
                    <h3 className="text-3xl md:text-2xl lg:text-3xl font-serif font-bold text-slate-900 mb-8 group-hover:text-sky-600 transition-colors leading-[1.15] tracking-tight">
                      {post.title}
                    </h3>
                    <div className="flex items-center gap-4 text-[9px] font-black text-slate-300 uppercase tracking-[0.4em] group-hover:text-slate-950 transition-colors">
            ...
            <div className="md:hidden flex items-center justify-center gap-4 mt-8">
              <div className="w-32 h-0.5 bg-slate-100 rounded-full overflow-hidden relative">
                <div className="absolute inset-0 bg-sky-500/20 animate-[pulse_3s_infinite]"></div>
                <div className="h-full bg-sky-500 w-1/3 transition-all duration-700 origin-left" style={{ transform: 'translateX(0%)' }}></div>
              </div>
            </div>
          </div>

          <div className="text-center mt-24 md:mt-40 reveal">
            <Link to="/stories" className="inline-flex items-center gap-8 group">
              <div className="relative">
                <span className="text-[11px] font-bold text-slate-950 uppercase tracking-[0.6em] border-b-2 border-slate-100 pb-1 group-hover:border-sky-500 transition-all duration-500">
                  Access the Full Archive
                </span>
              </div>
              <div className="w-16 h-16 rounded-full border border-slate-200 flex items-center justify-center group-hover:bg-slate-950 group-hover:border-slate-950 transition-all duration-700 shadow-sm">
                <svg className="w-6 h-6 text-slate-950 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
              </div>
            </Link>
          </div>
        </div>
