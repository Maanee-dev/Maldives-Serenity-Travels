...
const Home: React.FC = () => {
  ...
  const vibes = [
    ...
    { name: 'Adventure', desc: 'Dive into the deep blue or chase the perfect wave.', color: 'bg-orange-50', icon: '🐋' },
  ...
  return (
    <div className="bg-[#FCFAF7] selection:bg-sky-100 selection:text-sky-900 overflow-x-hidden">
      ...
      {/* Hero Section */}
      <section className="relative h-[100svh] w-full flex items-center justify-center overflow-hidden bg-sky-950">
        <div className="absolute inset-0 z-0">
          {heroSlides.map((slide, idx) => (
            <div key={idx} className={`absolute inset-0 transition-all duration-[3000ms] ease-out ${heroIndex === idx ? 'opacity-100 scale-100' : 'opacity-0 scale-110'}`}>
              ...
              <div className="absolute inset-0 bg-gradient-to-b from-sky-950/60 via-transparent to-sky-950/80" />
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-col items-start text-left">
            ...
            <div className="mb-16 reveal active">
               ...
               <div className="flex flex-col">
                ...
                <span className="text-6xl sm:text-8xl md:text-9xl lg:text-[13rem] font-serif font-bold text-white italic leading-[0.8] tracking-tighter opacity-90">{heroSlides[heroIndex].titleAlt}<span className="not-italic text-sky-400">.</span></span>
               </div>
            </div>
            <form onSubmit={handleSearch} className="w-full max-w-xl reveal active delay-500">
              <div className="relative group">
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={typedPlaceholder} className="w-full bg-white/10 backdrop-blur-3xl border border-white/20 rounded-full pl-10 pr-24 py-6 text-white text-[11px] font-bold uppercase tracking-[0.4em] outline-none focus:bg-white focus:text-sky-950 placeholder:text-white/30 shadow-2xl transition-all" />
                <button type="submit" className="absolute right-2 top-2 bottom-2 bg-sky-600 text-white w-16 rounded-full flex items-center justify-center hover:bg-sky-500 transition-all shadow-xl"><svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg></button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-20">
          <div className="flex flex-col lg:flex-row gap-16 lg:gap-32 items-center">
            <div className="lg:w-1/2 relative order-2 lg:order-1 reveal">
              <div className="relative aspect-[4/5] rounded-[3rem] overflow-hidden shadow-2xl z-10 group bg-sky-50">
                <img 
              ...
              <div className="absolute -bottom-10 -right-4 md:-bottom-16 md:-right-16 bg-[#FCFAF7] p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] shadow-2xl z-20 max-w-[280px] md:max-w-[380px] border border-sky-50 reveal delay-500">
                <p className="text-sky-950 font-serif italic text-xl md:text-3xl leading-[1.4]">"The profound happens in the gaps between the tides."</p>
                <div className="mt-8 flex items-center gap-4">
                  <div className="w-8 h-px bg-sky-500"></div>
                  <span className="text-[8px] font-bold uppercase tracking-widest text-sky-300">Maldivian Wisdom</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:w-1/2 order-1 lg:order-2 reveal">
              ...
              <h2 className="text-4xl md:text-7xl lg:text-8xl font-serif font-bold text-sky-950 leading-[0.95] mb-12 tracking-tighter">
                Luxury is <br /> 
                <span className="italic text-sky-500 font-normal">No Longer</span> <br /> 
                Optional.
              </h2>
              <div className="max-w-xl">
                <p className="text-sky-900/80 text-lg md:text-2xl leading-[1.7] mb-12 font-medium opacity-90 italic">
                  Defined by geography and refined by hand, we curate the silence that modern living lacks.
                </p>
                <p className="text-sky-950/60 text-base md:text-lg leading-[1.8] mb-12">
                ...
                <Link to="/about" className="inline-flex items-center gap-6 text-[10px] font-bold text-sky-950 uppercase tracking-[0.5em] group transition-all">
                  <span className="border-b-2 border-sky-100 pb-1 group-hover:border-sky-500 transition-colors">Our Full Narrative</span>
      ...
      {/* VIBE FINDER SECTION */}
      <section className="py-24 md:py-48 bg-sky-50/30">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-20 text-center mb-24 reveal">
           ...
           <h2 className="text-4xl md:text-7xl font-serif font-bold text-sky-950 italic tracking-tighter">Find Your Pulse.</h2>
        </div>
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-20 reveal">
          {vibes.map((v) => (
            <button 
              ...
              className={`p-10 rounded-[3rem] text-center transition-all duration-700 border-2 ${activeVibe === v.name ? 'bg-white border-sky-500 shadow-2xl scale-105' : 'bg-white border-transparent hover:border-sky-100'}`}
            >
              <div className="text-4xl mb-6">{v.icon}</div>
              <h3 className="text-[12px] font-black uppercase tracking-[0.4em] text-sky-950 mb-4">{v.name}</h3>
              <p className={`text-[10px] font-bold uppercase tracking-widest leading-loose ${activeVibe === v.name ? 'text-sky-600' : 'text-sky-300'}`}>
                {v.desc}
              </p>
            </button>
          ))}
        ...
        <div className="text-center reveal">
           <Link to={`/search?q=${activeVibe}`} className="bg-sky-600 text-white font-bold px-16 py-7 rounded-full text-[11px] uppercase tracking-[0.8em] hover:bg-sky-500 transition-all duration-700 shadow-2xl">
             Explore {activeVibe}
           </Link>
        </div>
      </section>
      ...
      {/* THE COLLECTION */}
      <section className="py-24 md:py-48 bg-white overflow-hidden border-b border-sky-50">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-20">
          <div className="mb-20 md:mb-32 reveal flex flex-col md:flex-row justify-between items-end gap-10">
            <div className="max-w-xl text-left">
              <span className="text-[10px] font-bold text-sky-400 uppercase tracking-[1.2em] mb-8 block">The Portfolio</span>
              <h3 className="text-4xl md:text-8xl font-serif font-bold text-sky-950 tracking-tighter italic leading-none">The Collection.</h3>
              <p className="mt-12 text-sky-300 text-[10px] uppercase font-bold tracking-[0.4em] leading-loose">
            ...
            <div className="w-24 h-[1px] bg-sky-200 mb-4 hidden md:block"></div>
          </div>
          <div className="reveal no-scrollbar overflow-x-auto flex gap-8 md:gap-16 pb-12 snap-x snap-mandatory">
            ...
            <div className="flex-shrink-0 w-[85vw] sm:w-[55vw] lg:w-[35vw] snap-start flex items-center justify-center">
              <Link to="/stays" className="group w-full aspect-[4/5] rounded-[3rem] bg-sky-50 border-2 border-dashed border-sky-100 flex flex-col items-center justify-center p-12 text-center hover:bg-sky-600 transition-all duration-1000">
                <span className="text-[10px] font-bold text-sky-300 group-hover:text-white uppercase tracking-[1em] mb-8 block">Explore All</span>
                <h4 className="text-2xl md:text-4xl font-serif font-bold text-sky-950 group-hover:text-white leading-tight italic">Find your <br /> sanctuary.</h4>
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 md:py-48 bg-sky-50/10">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-20">
          <div className="text-center mb-24 reveal">
            <span className="text-sky-500 font-black uppercase tracking-[1em] text-[10px] mb-8 block">The Journal</span>
            <h2 className="text-4xl md:text-7xl font-serif font-bold text-sky-950 italic tracking-tighter">Editorial Dispatches.</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-20 mb-24">
            {recentStories.map((post) => (
              <Link key={post.id} to={`/stories/${post.slug}`} className="group reveal">
                <div className="relative aspect-[3/4] rounded-[2.5rem] overflow-hidden mb-10 shadow-sm transition-all duration-1000 group-hover:shadow-2xl group-hover:-translate-y-2 bg-sky-50">
                  ...
                  <div className="absolute top-8 left-8">
                    <span className="bg-white/95 backdrop-blur-md px-5 py-2 rounded-full text-[9px] font-black text-sky-900 uppercase tracking-[0.4em] shadow-sm">
                      {post.category}
                    </span>
                  </div>
                </div>
                <div className="px-4">
                  <span className="text-sky-300 font-bold text-[9px] uppercase tracking-[0.4em] mb-4 block">
                    {new Date(post.date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                  </span>
                  <h3 className="text-2xl font-serif font-bold text-sky-950 mb-6 group-hover:text-sky-600 transition-colors leading-tight">
                    {post.title}
                  </h3>
                  <div className="h-px w-12 bg-sky-100 group-hover:w-full group-hover:bg-sky-500 transition-all duration-1000"></div>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="text-center reveal">
            <Link to="/stories" className="inline-flex items-center gap-6 text-[10px] font-bold text-sky-950 uppercase tracking-[0.5em] group transition-all">
              <span className="border-b-2 border-sky-50 pb-1 group-hover:border-sky-500 transition-colors">Access the Archives</span>
              <div className="w-14 h-14 rounded-full border border-sky-100 flex items-center justify-center group-hover:bg-sky-600 transition-all duration-700">
                <svg className="w-5 h-5 text-sky-950 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
              </div>
            </Link>
      ...
      {/* FINAL CTA */}
      <section className="py-24 md:py-48 bg-sky-900 relative overflow-hidden text-center text-white">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 reveal">
          <span className="text-[10px] font-bold text-sky-300 uppercase tracking-[1.5em] mb-12 block">Ready for Perspective?</span>
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16">
            <Link to="/plan" className="w-full md:w-auto bg-white text-sky-900 font-bold px-16 py-7 rounded-full hover:bg-sky-400 hover:text-white transition-all duration-700 uppercase tracking-[0.5em] text-[11px] shadow-2xl">
              Initiate Inquiry
            </Link>
          </div>
        </div>
