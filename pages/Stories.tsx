    ...
    <div className="bg-parchment dark:bg-slate-950 min-h-screen transition-colors duration-700">
      ...
      <section className="relative h-[60vh] md:h-[80vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 reveal active">
          ...
          <h1 className="text-6xl md:text-9xl font-serif font-bold text-white tracking-tighter italic leading-none">Editorial <br/> Archives.</h1>
          ...
          <p className="text-white text-[11px] font-bold max-w-xl mx-auto uppercase tracking-[0.5em] leading-[2.5] opacity-90">
             Narratives defining the silent luxury of the archipelago.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 py-24 md:py-32 lg:px-12">
        ...
        <div className="mb-24 flex flex-col md:flex-row justify-between items-center gap-12 border-b border-slate-100 dark:border-white/5 pb-12 reveal transition-colors">
          <div className="flex flex-wrap justify-center gap-8">
            {categories.map(cat => (
              <button
                ...
                className={`text-[10px] font-bold uppercase tracking-[0.4em] transition-all pb-2 border-b-2 ${activeCategory === cat ? 'border-sky-500 text-slate-900 dark:text-white' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
              >
                {cat}s
              </button>
            ))}
          ...
          <div className="relative w-full md:w-80">
            <input 
              ...
              className="w-full bg-transparent border-b border-slate-200 dark:border-white/10 py-2 text-[10px] font-bold uppercase tracking-widest outline-none focus:border-slate-950 dark:focus:border-white text-slate-950 dark:text-white placeholder:text-slate-300 dark:placeholder:text-slate-700 transition-all"
            />
          </div>
        </div>
        ...
        {loading ? (
          <div className="py-40 text-center reveal active">
             <div className="w-8 h-8 border-2 border-slate-100 dark:border-white/5 border-t-sky-500 rounded-full animate-spin mx-auto mb-8"></div>
             <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Accessing records...</p>
          </div>
                ...
                <Link to={`/stories/${featuredPost.slug}`} className="group flex flex-col reveal active">
                  <div className="relative overflow-hidden rounded-[3rem] md:rounded-[5rem] shadow-sm group-hover:shadow-2xl transition-all duration-1000 aspect-[21/9] mb-16 bg-slate-100 dark:bg-slate-900">
                     ...
                     <div className="absolute top-10 left-10">
                        <span className="bg-white/90 dark:bg-slate-900/90 backdrop-blur px-6 py-2 rounded-full text-[9px] font-bold text-slate-900 dark:text-white uppercase tracking-[0.4em] shadow-sm border dark:border-white/5">
                  ...
                  <div className="max-w-4xl mx-auto text-center">
                    ...
                    <h2 className="text-4xl md:text-7xl font-serif font-bold text-slate-900 dark:text-white mb-8 group-hover:italic transition-all duration-700 leading-[1.1] tracking-tighter">
                      {featuredPost.title}
                    </h2>
                    <p className="text-slate-600 dark:text-slate-400 leading-relaxed font-medium opacity-90 mb-10 text-lg md:text-xl italic transition-colors">
                ...
                >
                  <div className="relative overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] shadow-sm group-hover:shadow-xl transition-all duration-1000 aspect-[4/5] mb-10 bg-slate-100 dark:bg-slate-900">
                     ...
                     <div className="absolute top-6 left-6">
                        <span className="bg-white/90 dark:bg-slate-900/90 backdrop-blur px-4 py-1.5 rounded-full text-[8px] font-bold text-slate-900 dark:text-white uppercase tracking-widest shadow-sm border dark:border-white/5">
                           {post.category}
                        </span>
                     </div>
                  </div>
                  <div>
                    ...
                    <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed font-medium opacity-80 line-clamp-2 transition-colors">
                      {post.excerpt}
                    </p>
                  </div>
                </Link>
          ...
          <div className="py-40 text-center reveal active">
             <h3 className="text-3xl font-serif font-bold italic text-slate-900 dark:text-white mb-6">No archives found.</h3>
             <button onClick={() => {setActiveCategory('All'); setSearchQuery('');}} className="text-sky-500 font-bold uppercase tracking-widest text-[10px] border-b border-sky-200 dark:border-sky-900">Reset Filters</button>
          </div>
        )}
      </div>

      <section className="py-48 bg-white dark:bg-slate-950 border-t border-slate-50 dark:border-white/5 transition-colors">
         <div className="max-w-4xl mx-auto px-6 text-center reveal">
            ...
            <div className="flex flex-wrap justify-center gap-10">
               {['Aesthetics', 'Heritage', 'Sustainability', 'Cuisine'].map(tag => (
                 <button key={tag} className="text-[9px] font-bold text-slate-500 hover:text-slate-950 dark:hover:text-white uppercase tracking-[0.4em] transition-colors border-b border-transparent hover:border-slate-950 dark:hover:border-white pb-2">
                   {tag}
                 </button>
               ))}
            </div>
