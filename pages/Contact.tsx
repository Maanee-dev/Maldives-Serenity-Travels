        ...
        <div className="lg:col-span-5 space-y-24 reveal">
          <div className="space-y-12">
            <div>
              ...
              <div className="space-y-6">
                ...
                <a href="https://wa.me/9607775678" target="_blank" rel="noreferrer" className="flex items-center gap-6 group">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[11px] font-bold uppercase tracking-widest text-slate-900">+960 777 5678</span>
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Bespoke Concierge</span>
                  </div>
                </a>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-4">Digital Inquiry</span>
                <p className="text-[11px] font-bold text-slate-900 uppercase tracking-[0.2em]">info@maldives-serenitytravels.com</p>
              </div>
              <div>
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-4">Global Office</span>
                <p className="text-[11px] font-bold text-slate-900 uppercase tracking-[0.2em] leading-loose">
                  Faith, S.feydhoo<br />
                  Addu City, Maldives
                </p>
              </div>
        ...
        <div className="lg:col-span-7 reveal delay-300">
          {submitted ? (
            <div className="bg-white rounded-[4rem] p-20 text-center shadow-2xl border border-slate-50">
              <h2 className="text-4xl font-serif font-bold italic mb-6">Inquiry Received.</h2>
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.5em] leading-loose mb-12">Our specialists will review your vision and contact you within 24 hours.</p>
              <button onClick={() => setSubmitted(false)} className="text-[10px] font-black text-sky-500 uppercase tracking-[0.6em] border-b border-sky-100 pb-2">Send Another</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="bg-white rounded-[4rem] p-12 md:p-20 shadow-2xl border border-slate-50 space-y-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-4">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-4">Full Identity</label>
                  <input type="text" required className="w-full bg-slate-50 border-none rounded-full px-8 py-5 text-[11px] font-bold uppercase tracking-widest focus:ring-2 focus:ring-sky-500 transition-all placeholder:text-slate-200" placeholder="NAME" />
                </div>
                <div className="space-y-4">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-4">Email Channel</label>
                  <input type="email" required className="w-full bg-slate-50 border-none rounded-full px-8 py-5 text-[11px] font-bold uppercase tracking-widest focus:ring-2 focus:ring-sky-500 transition-all placeholder:text-slate-200" placeholder="EMAIL" />
                </div>
              </div>
              <div className="space-y-4">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-4">Your Vision</label>
                <textarea rows={6} className="w-full bg-slate-50 border-none rounded-[2.5rem] px-8 py-6 text-[11px] font-bold uppercase tracking-widest focus:ring-2 focus:ring-sky-500 transition-all placeholder:text-slate-200 resize-none" placeholder="TELL US ABOUT YOUR ATOL JOURNEY..."></textarea>
              </div>
              <button type="submit" className="w-full bg-slate-950 text-white font-black py-7 rounded-full text-[11px] uppercase tracking-[0.8em] hover:bg-sky-500 transition-all duration-700 shadow-2xl">
                Dispatch Inquiry
              </button>
            </form>
          )}
