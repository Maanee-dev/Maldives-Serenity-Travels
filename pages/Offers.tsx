  ...
  const StarRating = ({ count }: { count: number }) => (
    <div className="flex gap-0.5 mb-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} className={`w-4 h-4 ${i < count ? 'text-amber-400 fill-current' : 'text-slate-200 fill-current'}`} viewBox="0 0 24 24">
    ...
    <div className="bg-[#FCFAF7] min-h-screen selection:bg-sky-100 selection:text-sky-900 pb-32">
       <section className="pt-48 pb-16 px-6 text-center">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="text-6xl md:text-8xl font-serif font-bold text-slate-950 tracking-tighter leading-none mb-12">Bespoke <br /> <span className="italic text-slate-400 font-normal">Privileges.</span></h1>
          </div>
       </section>
       ...
       <section className="max-w-7xl mx-auto px-6 mb-20 sticky top-24 z-50">
          <div className="bg-white/80 backdrop-blur-xl border border-slate-100 rounded-[3rem] p-4 md:p-6 shadow-2xl flex flex-col lg:flex-row gap-6 items-center">
            <div className="flex-1 w-full relative">
              <input type="text" placeholder="SEARCH RESORT OR OFFER..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-50 border-none rounded-full px-8 py-4 text-[10px] font-black uppercase tracking-widest focus:ring-2 focus:ring-sky-500 transition-all placeholder:text-slate-300" />
            </div>
            ...
            <div className="flex items-center gap-4 px-4 border-l border-slate-100 hidden lg:flex">
               {categories.map(cat => (
                 <button key={cat} onClick={() => { setActiveCategory(cat); setCurrentPage(1); }} className={`text-[9px] font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'text-sky-500' : 'text-slate-400 hover:text-slate-900'}`}>{cat}</button>
               ))}
            </div>
          </div>
       </section>
       ...
       <section className="max-w-7xl mx-auto px-6">
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {paginatedOffers.map((offer) => (
                  <div key={offer.id} className="group flex flex-col h-full">
                    ...
                    <div className="px-2 flex flex-col h-full">
                       ...
                       <p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.3em] mb-4 leading-relaxed">{offer.resortName} • {offer.roomCategory}</p>
                       <h3 className="text-2xl font-serif font-bold text-slate-950 mb-6 leading-[1.2] group-hover:text-sky-500 transition-colors">{offer.title}</h3>
                       <div className="mt-auto pt-6 flex flex-col gap-1">
                          ...
                          <Link to={`/stays/${offer.resortSlug}`} className="mt-6 inline-flex items-center gap-4 text-[9px] font-black uppercase tracking-[0.5em] text-slate-900 group/btn transition-all">
                             <span className="border-b border-transparent group-hover/btn:border-slate-900 pb-1 transition-all">Refine Discovery</span>
                             <svg className="w-4 h-4 transform group-hover/btn:translate-x-2 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                          </Link>
                       </div>
                    </div>
                  </div>
            ...
            <div className="py-64 text-center">
               ...
               <button onClick={() => { setSearchQuery(''); setSelectedNights('All'); setActiveCategory('All'); setCurrentPage(1); }} className="text-sky-500 font-black uppercase tracking-widest text-[10px] border-b border-sky-100 pb-2">Reset Discovery Filters</button>
            </div>
          )}
       </section>
    </div>
