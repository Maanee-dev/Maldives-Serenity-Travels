// Implementation moved to uppercase filename to resolve casing conflicts.
              ...
              <div className="flex gap-2">
                {nightOptions.map(n => (
                  <button 
                    ...
                    className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${selectedNights === n ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-950' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                  >
                    {n === 'All' ? 'Any' : `${n} Days`}
                  </button>
                ))}
