...
const ResortDetail: React.FC = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FCFAF7] flex flex-col items-center justify-center">
       <div className="w-8 h-8 border-[1px] border-sky-100 border-t-sky-500 rounded-full animate-spin mb-8"></div>
       <p className="text-[10px] font-black text-sky-300 uppercase tracking-[0.4em]">Consulting the Atolls...</p>
    </div>
  );

  if (!resort) return (
    <div className="min-h-screen bg-[#FCFAF7] flex flex-col items-center justify-center px-6 text-center">
      <h2 className="text-3xl font-serif font-bold italic mb-6 text-sky-950 tracking-tighter">Sanctuary not found.</h2>
  ...
  return (
    <div className="bg-[#FCFAF7] min-h-screen selection:bg-sky-100 selection:text-sky-900 pb-20 overflow-x-hidden">
      ...
      <section className="relative w-full pt-20 md:pt-28 lg:pt-32 px-4 md:px-6 lg:px-12 reveal active">
        <div className="relative aspect-[4/5] md:aspect-[16/9] lg:aspect-[21/9] w-full rounded-[2.5rem] md:rounded-[3.5rem] lg:rounded-[4.5rem] overflow-hidden shadow-2xl bg-sky-50">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 md:p-12">
             ...
             <h1 className="text-3xl md:text-7xl lg:text-[8rem] font-serif font-bold text-white tracking-tighter italic leading-[1.1] drop-shadow-2xl reveal active delay-300">{resort.name}</h1>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-32 lg:py-48 px-6 lg:px-12">
        <div className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-20 lg:gap-32 items-start">
          <div className="lg:col-span-7 reveal">
            ...
            <p className="text-2xl md:text-5xl lg:text-6xl font-serif font-bold italic text-sky-950 leading-[1.2] tracking-tight mb-12 md:mb-16">
              "{resort.uvp}"
            </p>
            <div className="text-sky-900/80 text-base md:text-xl leading-[1.8] md:leading-[2.2] font-semibold space-y-6 md:space-y-8 max-w-3xl">
              <p>{resort.description}</p>
            </div>
          </div>
          <div className="lg:col-span-5 reveal active delay-500">
            <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-12 shadow-sm border-[1px] border-sky-100">
              <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-sky-950 mb-8 md:mb-12 border-b-[1px] border-sky-50 pb-4 md:pb-6">Essential Amenities</h3>
              <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 md:gap-6">
                {resort.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-4 group">
                    <div className="w-1.5 h-1.5 rounded-full bg-sky-400 group-hover:scale-[1.5] transition-transform flex-shrink-0"></div>
                    <span className="text-[11px] md:text-[12px] font-black text-sky-950 uppercase tracking-[0.2em]">{feature}</span>
                  </li>
                ))}
              </ul>
              <div className="mt-12 md:mt-16 pt-12 border-t-[1px] border-sky-50 grid grid-cols-2 gap-6 md:gap-8">
                <div>
                  <span className="text-[10px] font-black text-sky-300 uppercase tracking-widest mb-2 block">Arrival</span>
                  <p className="text-[11px] font-black text-sky-950 uppercase tracking-widest leading-relaxed">
                    {resort.transfers.map(t => t.replace(/_/g, ' ')).join(' • ')}
                  </p>
                </div>
                <div>
                  <span className="text-[10px] font-black text-sky-300 uppercase tracking-widest mb-2 block">Meal Plan</span>
                  <p className="text-[11px] font-black text-sky-950 uppercase tracking-widest leading-relaxed">
          ...
          <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
            <div className="mb-20 md:mb-32 reveal text-center lg:text-left flex flex-col lg:flex-row justify-between items-end gap-10">
              <div className="max-w-2xl">
                ...
                <h3 className="text-4xl md:text-7xl font-serif font-bold text-sky-950 tracking-tighter italic leading-none">Curated Journeys.</h3>
                <p className="mt-12 text-sky-300 text-[10px] uppercase font-black tracking-[0.4em] leading-loose">
                   Beyond the villa walls lies the archipelago. Movement that defines your unique Maldivian perspective at {resort.name}.
                </p>
              </div>
              <Link to={`/experiences`} className="inline-flex items-center gap-6 text-[10px] font-bold text-sky-950 uppercase tracking-[0.5em] group transition-all">
                <span className="border-b-2 border-sky-50 pb-1 group-hover:border-sky-500 transition-colors">Discover All Atolls</span>
                <div className="w-14 h-14 rounded-full border border-sky-100 flex items-center justify-center group-hover:bg-sky-600 transition-all duration-700">
                  <svg className="w-5 h-5 text-sky-950 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                </div>
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
              {experiences.map((exp, idx) => (
                <div key={exp.id} className="reveal group" style={{ transitionDelay: `${idx * 150}ms` }}>
                  <div className="relative aspect-[1/1] rounded-[2.5rem] overflow-hidden mb-10 shadow-sm transition-all duration-1000 group-hover:shadow-2xl bg-sky-50">
                    ...
                    <div className="absolute top-6 left-6">
                      <span className="bg-white/95 backdrop-blur px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest text-sky-900 shadow-sm">
                        {exp.category}
                      </span>
                    </div>
                  </div>
                  <div className="px-2">
                    <h4 className="text-xl md:text-2xl font-serif font-bold text-sky-950 mb-4 group-hover:text-sky-600 transition-colors">{exp.title}</h4>
                    <p className="text-sky-900/60 text-[12px] leading-relaxed mb-8 opacity-80 line-clamp-2">{exp.description}</p>
                    <button 
                      onClick={() => {
                      ...
                      className="text-[9px] font-black text-sky-950 border-b-2 border-sky-50 group-hover:border-sky-500 transition-all uppercase tracking-[0.4em] pb-1"
                    >
      ...
      <section id="inquiry-form" className="py-20 md:py-32 lg:py-48 bg-sky-900 text-white relative overflow-hidden">
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-start relative z-10">
           <div className="reveal text-center lg:text-left">
              ...
              <h3 className="text-4xl md:text-7xl lg:text-8xl font-serif font-bold italic mb-8 md:mb-12 tracking-tighter leading-tight">Send an Inquiry.</h3>
              {!isSubmitted && (
                <div className="flex items-center justify-center lg:justify-start gap-4 mb-12">
                  {[1, 2, 3].map(s => (
                    <React.Fragment key={s}>
                      <div className={`w-8 h-8 md:w-12 md:h-12 rounded-full flex items-center justify-center text-[10px] font-black transition-all duration-700 ${formStep === s ? 'bg-sky-500 text-white shadow-[0_0_25px_rgba(56,189,248,0.5)] scale-110' : formStep > s ? 'bg-sky-800 text-sky-500' : 'bg-sky-950/50 text-sky-300/30'}`}>
                        {s}
                      </div>
                      {s < 3 && <div className="w-6 md:w-10 h-px bg-sky-800"></div>}
                    </React.Fragment>
                  ))}
                </div>
              )}
              <p className="text-sky-200 text-lg md:text-xl lg:text-2xl leading-relaxed opacity-80 uppercase tracking-[0.2em] font-medium max-w-md mx-auto lg:mx-0">
           ...
           <div className="reveal delay-300 w-full">
             {isSubmitted ? (
               <div className="bg-white text-sky-950 p-12 md:p-20 rounded-[2.5rem] md:rounded-[3.5rem] text-center shadow-2xl animate-in zoom-in-95 duration-700">
                  ...
                  <p className="text-sky-300 text-[10px] font-black uppercase tracking-[0.4em] leading-[2.5] mb-12">Our travel experts will contact you within 24 hours.</p>
                  <button onClick={() => { setIsSubmitted(false); setFormStep(1); }} className="text-[10px] font-black text-sky-950 uppercase tracking-[0.6em] border-b-[2px] border-sky-950 pb-2 hover:text-sky-600 hover:border-sky-600 transition-colors">Submit Another</button>
               </div>
             ) : (
               <div className="bg-white/5 backdrop-blur-3xl p-6 md:p-12 lg:p-16 rounded-[2.5rem] md:rounded-[4rem] border border-white/10 shadow-2xl overflow-hidden">
                  {formStep === 1 && (
                    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-10 duration-700">
                          ...
                          <div className="space-y-2">
                             ...
                             <select className="w-full bg-white/10 border border-white/10 rounded-full px-8 py-5 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:bg-white/15 focus:border-white/30 transition-all text-white appearance-none cursor-pointer" value={quoteData.country} onChange={e => {
                               ...
                               setQuoteData({...quoteData, country: e.target.value, countryCode: selected?.code || '+44'});
                                ...
                                {COUNTRIES.map(c => <option key={c.name} value={c.name} className="bg-sky-900">{c.name}</option>)}
                             </select>
                          </div>
                          <div className="space-y-2">
                      ...
                      <button onClick={nextStep} className="w-full bg-white text-sky-900 font-black py-6 md:py-8 rounded-full text-[11px] uppercase tracking-[0.8em] hover:bg-sky-400 hover:text-white transition-all duration-700 shadow-2xl">NEXT: CHOOSE DATES</button>
                    </div>
                  )}
                  ...
                  {formStep === 2 && (
                    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-10 duration-700">
                       ...
                       <div className="flex flex-col sm:flex-row gap-4">
                          ...
                          <button onClick={nextStep} disabled={!quoteData.checkIn || !quoteData.checkOut} className="flex-[2] bg-white text-sky-900 font-black py-6 rounded-full text-[10px] uppercase tracking-[0.8em] hover:bg-sky-400 hover:text-white transition-all disabled:opacity-20">PROCEED</button>
                       </div>
                    </div>
                  )}
                  ...
                  {formStep === 3 && (
                    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-10 duration-700">
                      <div className="space-y-5">
                        <div className="space-y-2">
                           ...
                           <select className="w-full bg-white/10 border border-white/10 rounded-full px-8 py-5 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:bg-white/15 focus:border-white/30 transition-all text-white appearance-none cursor-pointer" value={quoteData.roomType} onChange={e => setQuoteData({...quoteData, roomType: e.target.value})}>
                                <option value="" className="bg-sky-900 text-white">SELECT A ROOM</option>
                                {resort.roomTypes?.map((r, i) => (
                                  <option key={i} value={r.name} className="bg-sky-900 text-white">{r.name}</option>
                                ))}
                           </select>
                        </div>
                        <div className="space-y-2">
                           ...
                           <select className="w-full bg-white/10 border border-white/10 rounded-full px-8 py-5 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:bg-white/15 focus:border-white/30 transition-all text-white appearance-none cursor-pointer" value={quoteData.mealPlan} onChange={e => setQuoteData({...quoteData, mealPlan: e.target.value})}>
                                <option value="" className="bg-sky-900 text-white">SELECT MEAL PLAN</option>
                                {resort.mealPlans?.map((m, i) => (
                                  <option key={i} value={m} className="bg-sky-900 text-white">{m.replace(/_/g, ' ')}</option>
                                ))}
                           </select>
                        </div>
                        <div className="space-y-2">
                      ...
                      <div className="flex flex-col sm:flex-row gap-4">
                        <button onClick={prevStep} className="flex-1 border border-white/10 text-white font-black py-6 rounded-full text-[10px] uppercase tracking-[0.4em] hover:bg-white/10 transition-all">BACK</button>
                        <button onClick={handleQuoteSubmit} disabled={isSubmitting} className="flex-[2] bg-white text-sky-900 font-black py-6 rounded-full text-[10px] uppercase tracking-[0.8em] hover:bg-sky-400 hover:text-white transition-all duration-700 shadow-2xl disabled:opacity-50">
      ...
      {similarStays.length > 0 && (
        <section className="py-24 md:py-48 bg-[#FCFAF7] border-t-[1px] border-sky-50">
          <div className="max-w-[1440px] mx-auto">
            <div className="px-6 lg:px-12 mb-16 md:mb-24 reveal">
              <span className="text-[11px] font-black text-sky-300 uppercase tracking-[0.8em] mb-4 block">Refined Selection</span>
              <h3 className="text-3xl md:text-6xl font-serif font-bold italic text-sky-950 tracking-tighter">Similar Sanctuaries.</h3>
            </div>
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-16 border-t border-sky-100 flex flex-col md:flex-row justify-between items-center gap-6 reveal">
         <Link to="/stays" className="text-[10px] font-black text-sky-300 uppercase tracking-[0.6em] hover:text-sky-950 transition-colors flex items-center gap-4">
           <span className="text-lg">←</span> Return to Portfolio
         </Link>
         <span className="text-[10px] font-black text-sky-200 uppercase tracking-[0.4em] text-center">Defined by Perspective • 2026 Archive</span>
      </div>
