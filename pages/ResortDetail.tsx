...
const ResortDetail: React.FC = () => {
      ...
      try {
        ...
        if (resData) {
          ...
          const mappedResort: Accommodation = {
            ...
            uvp: resData.uvp || localBackup?.uvp || 'A beautiful hotel for your next trip.',
  ...
  const handleQuoteSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('We had an error sending your message. Please try again.');
    } finally {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FCFAF7] flex flex-col items-center justify-center">
       ...
       <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Loading Resort Info...</p>
    </div>
  );

  if (!resort) return (
    <div className="min-h-screen bg-[#FCFAF7] flex flex-col items-center justify-center px-6 text-center">
      <h2 className="text-3xl font-serif font-bold italic mb-6 text-slate-900 tracking-tighter">Resort not found.</h2>
      <Link to="/stays" className="text-sky-600 font-black uppercase tracking-[0.5em] text-[11px] underline underline-offset-8">Back to Hotels</Link>
    </div>
  );
    ...
    <div className="bg-[#FCFAF7] min-h-screen selection:bg-sky-100 selection:text-sky-900 pb-20 overflow-x-hidden">
        ...
        <div className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-20 lg:gap-32 items-start">
          <div className="lg:col-span-7 reveal">
            <div className="flex items-center gap-6 mb-8 md:mb-12">
              ...
              <span className="text-[11px] font-black text-sky-600 uppercase tracking-[0.8em] block">About the Resort</span>
            </div>
          ...
          <div className="lg:col-span-5 reveal active delay-500">
            <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-12 shadow-sm border-[1px] border-slate-100">
              <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-950 mb-8 md:mb-12 border-b-[1px] border-slate-50 pb-4 md:pb-6">Main Features</h3>
              ...
              <div className="mt-12 md:mt-16 pt-12 border-t-[1px] border-slate-50 grid grid-cols-2 gap-6 md:gap-8">
                <div>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Travel Method</span>
                ...
                <div>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Food Plans</span>
                  <p className="text-[11px] font-black text-slate-900 uppercase tracking-widest leading-relaxed">
          ...
          <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
            <div className="mb-20 md:mb-32 reveal text-center lg:text-left flex flex-col lg:flex-row justify-between items-end gap-10">
              <div className="max-w-2xl">
                <span className="text-[11px] font-black text-sky-500 uppercase tracking-[1em] mb-8 block">Fun Activities</span>
                <h3 className="text-4xl md:text-7xl font-serif font-bold text-slate-900 tracking-tighter italic leading-none">Things To Do.</h3>
                <p className="mt-12 text-slate-400 text-[10px] uppercase font-black tracking-[0.4em] leading-loose">
                   Enjoy the best activities the Maldives has to offer. We help you book fun tours and experiences at {resort.name}.
                </p>
              </div>
              <Link to={`/experiences`} className="inline-flex items-center gap-6 text-[10px] font-bold text-slate-950 uppercase tracking-[0.5em] group transition-all">
                <span className="border-b-2 border-slate-100 pb-1 group-hover:border-sky-500 transition-colors">See All Activities</span>
      ...
      <section id="inquiry-form" className="py-20 md:py-32 lg:py-48 bg-slate-950 text-white relative overflow-hidden">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-start relative z-10">
           <div className="reveal text-center lg:text-left">
              <span className="text-[11px] font-black text-sky-400 uppercase tracking-[1em] mb-8 md:mb-12 block">Book Your Trip</span>
              <h3 className="text-4xl md:text-7xl lg:text-8xl font-serif font-bold italic mb-8 md:mb-12 tracking-tighter leading-tight">Send Us a Message.</h3>
              {!isSubmitted && (
                <div className="flex items-center justify-center lg:justify-start gap-4 mb-12">
                  {[1, 2, 3].map(s => (
                    <React.Fragment key={s}>
                      <div className={`w-8 h-8 md:w-12 md:h-12 rounded-full flex items-center justify-center text-[10px] font-black transition-all duration-700 ${formStep === s ? 'bg-sky-500 text-white shadow-xl scale-110' : formStep > s ? 'bg-slate-800 text-sky-500' : 'bg-slate-900 text-slate-600'}`}>
                        {s}
                      </div>
              ...
              <p className="text-slate-400 text-lg md:text-xl lg:text-2xl leading-relaxed uppercase tracking-[0.2em] font-medium max-w-md mx-auto lg:mx-0">
                {formStep === 1 ? "Tell us your contact info." : formStep === 2 ? "When do you want to travel?" : "Choose your stay options."}
              </p>
           </div>
           ...
           <div className="reveal delay-300 w-full">
             {isSubmitted ? (
               <div className="bg-white text-slate-950 p-12 md:p-20 rounded-[2.5rem] md:rounded-[3.5rem] text-center shadow-2xl animate-in zoom-in-95 duration-700">
                  ...
                  <h4 className="text-3xl md:text-4xl font-serif font-bold italic mb-6">Message Sent.</h4>
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] leading-[2.5] mb-12">Our team will message you back within 24 hours.</p>
                  <button onClick={() => { setIsSubmitted(false); setFormStep(1); }} className="text-[10px] font-black text-slate-950 uppercase tracking-[0.6em] border-b-[2px] border-slate-950 pb-2 hover:text-sky-600 hover:border-sky-600 transition-colors">Send Another Message</button>
               </div>
             ) : (
                    ...
                    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-10 duration-700">
                      <div className="space-y-4 md:space-y-6">
                        <div className="space-y-2">
                           <label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-4">Your Full Name</label>
                  ...
                  {formStep === 3 && (
                    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-10 duration-700">
                      <div className="space-y-5">
                        <div className="space-y-2">
                           <label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-4">Choose a Room?</label>
                        ...
                        <div className="space-y-2">
                           <label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-4">Food Plan?</label>
                           <select className="w-full bg-white/10 border border-white/10 rounded-full px-8 py-5 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:bg-white/15 focus:border-white/30 transition-all text-white appearance-none cursor-pointer" value={quoteData.mealPlan} onChange={e => setQuoteData({...quoteData, mealPlan: e.target.value})}>
                                <option value="" className="bg-slate-900 text-white">SELECT FOOD PLAN</option>
                        ...
                        <div className="space-y-2">
                           <label className="text-[9px] font-black text-white/40 uppercase tracking-widest ml-4">Any extra requests?</label>
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-16 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 reveal">
         <Link to="/stays" className="text-[10px] font-black text-slate-400 uppercase tracking-[0.6em] hover:text-slate-950 transition-colors flex items-center gap-4">
           <span className="text-lg">←</span> Back to All Hotels
         </Link>
         <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em] text-center">Maldives Serenity Travels • 2026</span>
      </div>
